...
print(calculate_by_distance_angle(-500,0))
...
class SpaceObject:
    ...
    def calculate_new_velocity(self, other_object, fps: int):
        ...
        x_distance = abs(self.x - other_object.x)
        y_distance = abs(self.y - other_object.y)
        acceleration = (self.G * other_object.mass) / pow(distance, 2)  # a = GM/R^2
        added_velocity = acceleration * (1 / fps)  # [a] = m/s^2 | 60 FPS => 1/60 * a
        x_velocity = self.x * math.cos(self.angle)
        y_velocity = self.y * math.sin(self.angle)
        ...
        print("HERE" + f"{resultant_angle} pi for {self.name}")
